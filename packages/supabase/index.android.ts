...
type RealtimeClient = SupabaseRealtimeClient & {
  native: org.nativescript.supabase.SupabaseChannel;
};

function parseCount(count?: 'exact' | 'planned' | 'estimated') {
  switch (count) {
    case 'exact':
      return (<any>org).nativescript.supabase.SupabaseClient.CountOption.exact;
    case 'planned':
      return (<any>org).nativescript.supabase.SupabaseClient.CountOption.planned;
    case 'estimated':
      return (<any>org).nativescript.supabase.SupabaseClient.CountOption.estimated;
    default:
      return null;
  }
}
...
export class SupabaseClient {
  native: org.nativescript.supabase.SupabaseClient;
  static fromNative(native: org.nativescript.supabase.SupabaseClient): SupabaseClient {
...
export function createClient(supabaseURL: string, supabaseKey: string) {
  try {
    const client = org.nativescript.supabase.SupabaseClient.createClient(supabaseURL, supabaseKey);
