...
import type { PresenceOpts, PresenceOnJoinCallback, PresenceOnLeaveCallback } from './lib/phoenix-types';
...
export default class RealtimePresence {
   ...
   * @internal
   */
  private static syncState(currentState: RealtimePresenceState, newState: RawPresenceState | RealtimePresenceState, onJoin: PresenceOnJoinCallback, onLeave: PresenceOnLeaveCallback): RealtimePresenceState {
    ...
    this.map(transformedState, (key, newPresences: Presence[]) => {
      ...
      if (currentPresences) {
        ...
        const joinedPresences: Presence[] = newPresences.filter((m: Presence) => curPresenceRefs.indexOf(m.presence_ref) < 0);
        const leftPresences: Presence[] = currentPresences.filter((m: Presence) => newPresenceRefs.indexOf(m.presence_ref) < 0);
   ...
   * @internal
   */
  private static syncDiff(state: RealtimePresenceState, diff: RawPresenceDiff | PresenceDiff, onJoin: PresenceOnJoinCallback, onLeave: PresenceOnLeaveCallback): RealtimePresenceState {
    const { joins, leaves } = {
    ...
    this.map(joins, (key, newPresences: Presence[]) => {
      ...
      if (currentPresences.length > 0) {
        ...
        const curPresences: Presence[] = currentPresences.filter((m: Presence) => joinedPresenceRefs.indexOf(m.presence_ref) < 0);
    ...
    this.map(leaves, (key, leftPresences: Presence[]) => {
      ...
      currentPresences = currentPresences.filter((m: Presence) => presenceRefsToRemove.indexOf(m.presence_ref) < 0);
   ...
   * @internal
   */
  private static transformState(state: RawPresenceState | RealtimePresenceState): RealtimePresenceState {
